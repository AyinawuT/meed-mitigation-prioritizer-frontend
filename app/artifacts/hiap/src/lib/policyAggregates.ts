 ...
 *
 * Shared so the Recommendations "Context breakdown" tab and the Policy
 * Alignment page derive the same regional/municipal numbers from the
 * action-policy-scores endpoint. National is the mean policy_support_score;
 * regional/municipal are the mean of the strongest supporting evidence per
 * action at that scope (null when no plan covers it).
 */

export type PolicyScope = "National" | "Regional" | "Municipal";

interface PolicyEvidenceLike {
  signal_strength?: string;
  document_type?: string;
  evidence_strength?: number;
}
interface PolicyScoreLike {
...
export function computePolicyAggregates(scores: PolicyScoreLike[]): PolicyAggregates {
