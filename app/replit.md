...
Note: Replit was the original hosting target. The current deployment target is Kubernetes via GitHub Actions and the `k8s/` manifests in this repository.
...
See the npm workspace configuration in `package.json` for workspace structure and shared tooling.
